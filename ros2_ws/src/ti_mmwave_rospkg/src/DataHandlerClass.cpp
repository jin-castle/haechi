...
static sensor_msgs::msg::PointField makePointField(
...
static void fillPointCloud2FromMmWaveCloud(
    const pcl::PointCloud<mmWaveCloudType> & cloud,
    sensor_msgs::msg::PointCloud2 & msg)
{
    const size_t point_count = cloud.points.size();
    msg.height = cloud.height > 0 ? cloud.height : 1;
    if (msg.height > 1 && cloud.width * cloud.height == point_count) {
    ...
    msg.fields = {
        ...
        makePointField("y", 4),
        makePointField("z", 8),
        makePointField("intensity", 12),
        makePointField("velocity", 16),
    };
    ...
    for (size_t i = 0; i < point_count; ++i) {
        const mmWaveCloudType & point = cloud.points[i];
...
static void appendRadarScanPoint(
    std::vector<mmWaveCloudType> & points,
    const ti_mmwave_rospkg_msgs::msg::RadarScan & scan)
{
    mmWaveCloudType point;
    point.x = scan.x;
    point.y = scan.y;
    point.z = scan.z;
    point.intensity = scan.intensity;
    point.velocity = scan.velocity;
...
void *DataUARTHandler::sortIncomingData(void)
{
    ...
    std::vector<mmWaveCloudType> frameCloudPoints;
    ...
    while(rclcpp::ok())
    {
        ...
        case READ_OBJ_STRUCT:
            ...
            // CHECK_TLV_TYPE code has already read tlvType and tlvLen
                ...
                // Populate pointcloud
                // std::cout << "I found " << mmwData.numObjOut << "points!" << std::endl;
                ...
                while( i < mmwData.numObjOut ) {
                    ...
                    { // SDK version is 3.x+
                        ...
                        //         << "y: " << RScan->points[i].y << " / "
                        //         << "z: " << RScan->points[i].z << " / "
                        //         << "velocity: " << RScan->points[i].velocity << std::endl;
                        ...
                        appendRadarScanPoint(frameCloudPoints, radarscan);

                        // mean_x += RScan->points[i].x;
                        // mean_y += RScan->points[i].y;
                        // mean_z += RScan->points[i].z;
                ...
                // [Haechi test 260110] delta sensor position move 
                if (mmwData.numObjOut >= 5)
                {
                    mean_x = mean_x / mmwData.numObjOut;
                    mean_y = mean_y / mmwData.numObjOut;
                    mean_z = mean_z / mmwData.numObjOut;

                    if (this->pre_mean_x == 0.0)
                    {
                        this->initial_pos_x = mean_x;
                        this->initial_pos_y = mean_y;
                        this->initial_pos_z = mean_z;
                    }

                    float delta_x = pre_mean_x - mean_x;
                    float delta_y = pre_mean_y - mean_y;
                    float delta_z = pre_mean_z - mean_z;

                    float cur_pos_x = mean_x - this->initial_pos_x;
                    float cur_pos_y = mean_y - this->initial_pos_y;
                    float cur_pos_z = mean_z - this->initial_pos_z;

                    // std::cout << "mean_x: " << mean_x << " / "
                    //     << "mean_y: " << mean_y << " / "
                    //     << "mean_z: " << mean_z << std::endl;

                    // std::cout << "pre_mean_x: " << pre_mean_x << " / "
                    //     << "pre_mean_y: " << pre_mean_y << " / "
                    //     << "pre_mean_z: " << pre_mean_z << std::endl;

                    // std::cout << "delta_x: " << delta_x << " / "
                    //     << "delta_y: " << delta_y << " / "
                    //     << "delta_z: " << delta_z << std::endl;

                    std::cout << "cur_pos_x: " << cur_pos_x << " / "
                        << "cur_pos_y: " << cur_pos_y << " / "
                        << "cur_pos_z: " << cur_pos_z << std::endl;

                    this->pre_mean_x = mean_x;
                    this->pre_mean_y = mean_y;
                    this->pre_mean_z = mean_z;
                }
        ...
        case CHECK_TLV_TYPE:
            {
                ...
                if(tlvCount++ >= mmwData.header.numTLVs)  // Done parsing all received TLV sections
                {
                    ...
                    if (!frameCloudPoints.empty()) {
                        RScan->height = 1;
                        RScan->width = static_cast<uint32_t>(frameCloudPoints.size());
                        RScan->is_dense = 1;
                        RScan->points.assign(frameCloudPoints.begin(), frameCloudPoints.end());
                    }
                    fillPointCloud2FromMmWaveCloud(*RScan, *pc2_msg_);
